#!/bin/bash
# ==============================================================================
# EC2 User Data Script - Flask Application Setup
# ==============================================================================
# This script runs automatically on first boot to install and configure the
# Flask web application with MySQL database connectivity
# ==============================================================================
...
# ------------------------------------------------------------------------------
# Create dedicated directory for Flask application
...
# ------------------------------------------------------------------------------
# Create isolated Python environment for dependency management
python3 -m venv venv
source venv/bin/activate

# Install Flask web framework and MySQL connector
pip install flask mysql-connector-python

# ------------------------------------------------------------------------------
# Flask Application Creation
# ------------------------------------------------------------------------------
# Generate Flask application file with database integration
cat > app.py << 'APPEOF'
...
# ------------------------------------------------------------------------------
# Connection parameters injected from Terraform variables
DB_CONFIG = {
    "host": "${db_host}",
    "user": "${db_username}",
...
def get_db_connection():
    """Establish database connection with automatic retry mechanism for resilience"""
    retries = 5
    while retries > 0:
        try:
...
def home():
    """Main page - displays messages and handles form submission"""
    conn = get_db_connection()
    messages = []
    error = None
...
@app.route("/db-info")
def db_info():
    """Database information endpoint - displays MySQL version and connection details"""
    try:
        connection = get_db_connection()
        if connection and connection.is_connected():
...
if __name__ == "__main__":
    # Run Flask app on all interfaces, port 80 for HTTP access
...
# ------------------------------------------------------------------------------
# Create service unit file for automatic startup and management
...
# Service Activation
# ------------------------------------------------------------------------------
# Reload systemd, enable service to start on boot, and start immediately
